function f = car_sddp_gazebov2()
%CAR_SDDP Example of trajectory optimization for car model in gazebo
% 
%This is second version of rccar optimization. In this version, steering angles are controlled and driving velocities are controlled through servos.
%It has 4 states x,y,theta,velocity 
%
...
S.sim.AttachServo([2,4],[100.0;100.0;50.0],[100;-100;500;-500]);%Attach a servo to control steering angles
...
% initial state 
x0 = [0; 0; 0; 0];
S.xf = [1.5; 0.2; 0; 0];%Posn(x,y),Angle, Body Velocity
%S.xf = [1.5; 2; pi/2; 0];%Posn(x,y),Angle, Body Velocity
...
% us(1,:) = 0.3;
% us(2,:) = 0.02;
us = [repmat([.2;0.1], 1, S.N/2), repmat([.2;0], 1, S.N/2)];
...
[LinkData, ~] = S.sim.Step((S.ts(k+1) - S.ts(k)), us);%Run Gazebo Dynamics
x = zeros(4,1);
