...
%   cyton_servo_control     - cyton_servo_control Control Cyton arm using servos.
...
%   arm_sddp_analytic       - Trajectory Optimization for 2DOF pendulum using SDDP Optimization (Analytic Model)
%   arm_sddp_gazebo         - ARM_SDDP Example of trajectory optimization for 2 link arm using SDDP
%   car_sddp_analytic       - Trajectory Optimization for rccar using SDDP Optimization (Analytic Model)
%   car_sddp_gazebov1       - CAR_SDDP Example of trajectory optimization for car model in gazebo
%   car_sddp_gazebov2       - CAR_SDDP Example of trajectory optimization for car model in gazebo
%   car_sddp_gazebov3       - CAR_SDDP Example of trajectory optimization for car model in gazebo
%   
...
%   pqp_example             - PQP Testing using mex interface:
%
% Helper Classes:
...
%   EvaluateJacobians       - EVALUATECOSTANDJACOBIANS: Evaluates the cost and jacobians by perturbing the controls and fitting a linear model to the dynamics
%   MatlabPqpManager        - Loads Pqp meshes and provides routines to compute distances between two meshes. 
%   sddp                    - Second-order numerical optimal control. (Sampling version of DDP)